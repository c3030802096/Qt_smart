    ...
    ui(new Ui::device_curtain)
{
    ...
    ui->label_curtain->setStyleSheet("border-image:url(:/png/baiyechuang_yezi_guan.png)");
    ui->label_curtain->setText("");

    ui->label_lan->setFixedHeight(15);
    ui->label_lan->setStyleSheet("border-image:url(:/png/baiyechuang_lan_guan.png)");
    ui->label_lan->setText("");

    ui->btn_up->setStyleSheet("border-image:url(:/png/shang_guan.png)");
    ui->btn_down->setStyleSheet("border-image:url(:/png/xia_guan.png)");
}
...
void DeviceCurtain::on_btn_up_pressed()
{
    ...
    //更新当前百分比
    currentPercentage=abs(end_y)*100/ui->label_curtain->height();
}
...
void DeviceCurtain::slot_slider_ON(void)
{
    ...
    ui->label_curtain->setStyleSheet("border-image:url(:/png/kaiyechuang_yezi_kai.png)");
    ui->label_lan->setStyleSheet("border-image:url(:/png/baiyechuang_lan_kai.png)");

    ui->btn_up->setStyleSheet("border-image:url(:/png/shang_kai.png)");
    ui->btn_down->setStyleSheet("border-image:url(:/png/xia_kai.png)");
...
void DeviceCurtain::openCurtain()
{
    qDebug() << "openCurtain 函数被调用";
    slot_slider_ON();
}



void DeviceCurtain::closeCurtain()
{
    qDebug() << "closeCurtain 函数被调用";
    slot_slider_OFF();
}
void DeviceCurtain::setCurtainPercentage(int percentage)//设置百叶窗打开百分比
{
    if(percentage<0||percentage>100){
        qDebug()<<"百分比无效"<<percentage;
        return;
    }
    currentPercentage=percentage;
    int start_x=ui->label_curtain->pos().x();
    int start_y=ui->label_curtain->pos().y();
    int end_x=ui->label_curtain->pos().x();
    int end_y=0-ui->label_curtain->height()*percentage/100;//根据百分比计算动画结束y坐标
    int run_time=abs(start_y-end_y)/5.0*200;//动画持续时间
    ...
    //启动定时器
    data_timer->start(100);
    qDebug()<<"百叶窗设置为打开："<<percentage;


}

int DeviceCurtain::getCurtainPercentage() const
{
    // 假设当前百分比存储在一个成员变量中
    return currentPercentage; // 需要在类中定义 `currentPercentage` 并在设置百分比时更新它
}

bool DeviceCurtain::isCurOn()const
{
    return curStatus;
}
